...
from MFFM import MFFM,MPI
from MCIM import MCIM
...
class RASnet(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, base_n_filter=16):
        ...
        self.feature_channels = feature_channels    #channel
        ...
        self.position_encoding1 = LearnedPositionalEncoding(405, self.feature_channels, seq_length)
        self.pe_dropout = nn.Dropout(p=0.2)
        self.pre_head_ln1 = nn.LayerNorm(self.feature_channels)

        self.transformer1 = TransformerModel(self.feature_channels,depth,heads,mlp_dim,0.1,0.1)

        self.deconv0 = nn.Conv3d(self.feature_channels, self.base_n_filter * 16, kernel_size=3, padding=1)
        self.relu1 = nn.ReLU(inplace=True)
        self.norm4 = nn.BatchNorm3d(256)
        self.cont = nn.Conv3d(self.base_n_filter*16,self.feature_channels,kernel_size=3,padding=1)

        #self.maxpool = nn.MaxPool3d(2)
        self.maxpool = nn.AvgPool3d(3,2)
        ...
        self.MCIM_4 = MCIM(self.base_n_filter*8)
    ...
    def forward(self, x):
        ...
        out = out.view(out.size(0), -1, self.feature_channels)
        ...
        out = out.view(out.size(0), out4.shape, self.feature_channels)     #shape
